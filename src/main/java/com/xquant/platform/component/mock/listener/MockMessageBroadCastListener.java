...
@Component
public class MockMessageBroadCastListener extends MessageMockManager
		implements ApplicationListener<MockMessageBroadCastSuccEvent> {
	 ...
	 * 以下成功动作导致报价成为终态
	 */
	private static final EnumSet<OpBitEnum> FINAL_STATE_OPBIT = EnumSet.of(OpBitEnum.CANCEL, OpBitEnum.CONFIRM,
			OpBitEnum.REJECT, OpBitEnum.EXPIRE);

	/**
	 * 具有交易对手的报价类型
	 */
	private static final EnumSet<QuoteBizTypeEnum> HAS_COUNTER_BIZTYPE = EnumSet.of(QuoteBizTypeEnum.BOND_DLG,
			QuoteBizTypeEnum.PLEDGEREPO_DLG, QuoteBizTypeEnum.BOND_RFQ_REQ, QuoteBizTypeEnum.BOND_RFQ_REPLY);
	...
	public void onApplicationEvent(MockMessageBroadCastSuccEvent event) {
		...
		if (logger.isInfoEnabled()) {
			logger.info("send asynchronous message success = " + originMessageInfo.getQuoteId() + ", optype ="
					+ originMessageInfo.getOptype() + ", delay = " + originMessageInfo.getDelay() + ", timeUint = "
					+ originMessageInfo.getTimeUnit().toString());
		...
		try {
			...
			if (CfetsConstantUtil.useCenterServer() && hasCounter(originMessageInfo)) {
				NettyLongLiveClientHolder.getClient().sendMessage(
						ProtocolEntitySerializeUtil.serialize((CfetsTradeMessage) originMessageInfo.getMessage()));
			}
			...
			if (CfetsConstantUtil.counterRfqMessageAutoReply()
					&& OpTypeEnum.BOND_RFQREQ_ADD.equals(originMessageInfo.getOptype())) {
				threadService.execute(new Runnable() {
					@Override
					public void run() {
			...
			if (CfetsConstantUtil.counterDlgMessageAutoReply()
					&& OpTypeEnum.BOND_DIALOGUE_ADD.equals(originMessageInfo.getOptype())) {
				threadService.execute(new Runnable() {
					@Override
					public void run() {
			...
			if (CfetsConstantUtil.counterDlgMessageAutoReply()
					&& OpTypeEnum.PLEDGEREPO_DIALOGUE_ADD.equals(originMessageInfo.getOptype())) {
				threadService.execute(new Runnable() {
					@Override
					public void run() {
	...
	private void buildExpiredTask(MockMessageInfo originMessageInfo) {
		...
		MockMessageInfo messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, originMessage,
				OpTypeEnum.getOpTypeEnum(expireOptype), null);
		// 进行消息的处理(入库和发送)
	...
	private void buildReplyMessageIfSelfBondRepoAdd(MockMessageInfo originMessageInfo) {
		if (CfetsTradeCollateralDialogueQuoteResMessage.class.isInstance(originMessageInfo.getMessage())) {
			CfetsTradeCollateralDialogueQuoteResMessage orignRfqMessage = (CfetsTradeCollateralDialogueQuoteResMessage) originMessageInfo
					.getMessage();
			CfetsTradeCollateralDialogueQuoteResMessage messageTempldate = ResourceUtil4Cfets
			...
			if (num < 0.8) {
				messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, orignRfqMessage,
						OpTypeEnum.PLEDGEREPO_DIALOGUE_CONFIRM, triggerTime);
			} else {
				messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, orignRfqMessage,
						OpTypeEnum.PLEDGEREPO_DIALOGUE_REJECT, triggerTime);
			}
			if (messageInfo != null) {
				executeTask(messageInfo);
			}
	...
	private void buildReplyMessageIfSelfBondDlgAdd(MockMessageInfo originMessageInfo) {
		if (CfetsTradeBondDialogueQuoteResMessage.class.isInstance(originMessageInfo.getMessage())) {
			CfetsTradeBondDialogueQuoteResMessage orignRfqMessage = (CfetsTradeBondDialogueQuoteResMessage) originMessageInfo
					.getMessage();
			...
			if (num < 0.8) {
				messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, orignRfqMessage,
						OpTypeEnum.BOND_DIALOGUE_CONFIRM, triggerTime);
			} else {
				messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, orignRfqMessage,
						OpTypeEnum.BOND_DIALOGUE_REJECT, triggerTime);
			}
			if (messageInfo != null) {
				executeTask(messageInfo);
			}
	...
	private void buildReplyMessageIfSelfRfqAdd(MockMessageInfo originMessageInfo) {
		if (CfetsTradeBondRFQResMessage.class.isInstance(originMessageInfo.getMessage())) {
			...
			MockMessageInfo messageInfo = messageMessageInfoBuilderManager.build(messageTempldate, null,
					OpTypeEnum.BOND_RFQREPLY_ADD, triggerTime);
			// 进行消息的处理(入库和发送)
			executeTask(messageInfo);
		}
	}
	...
	private void handleFinalStateMessage(@SuppressWarnings("rawtypes") MockMessageInfo originMessageInfo) {
		...
		if (QuoteBizTypeEnum.BOND_RFQ_REQ.equals(originMessageInfo.getOptype().getQuoteBizType())) {
			...
			List<String> relationSets = CommonCfetsTradeRfqResMessageDao
					.queryValidQuoteIdViaQuoteReqId(originMessageInfo.getQuoteId());
			if (CollectionUtils.isNotEmpty(relationSets)) {
				for (String replyQuoteId : relationSets) {
					...
					if (logger.isInfoEnabled()) {
						logger.info("remove reply quote which quoteId = " + replyQuoteId + " and quoteReqId = "
								+ originMessageInfo.getQuoteId());
					}
					@SuppressWarnings("rawtypes")
	...
	static {
		 ...
		 * 本方新增对话报价过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondDialogueQuoteResMessage",
				ResourceUtil4Cfets.getBondDlgResMessage4Expire());
		/**
		 * 对手对话报价过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondDialogueQuoteReceiveMessage",
				ResourceUtil4Cfets.getBondDlgRecvMessage4Expire());
		/**
		 * 本方质押式回购过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeCollateralDialogueQuoteResMessage",
				ResourceUtil4Cfets.getPledgeDlgResMessage4Expire());
		/**
		 * 对手质押式回购过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeCollateralDialogueQuoteReceiveMessage",
				ResourceUtil4Cfets.getPledgeDlgRecvMessage4Expire());
		/**
		 * 本方请求报价过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondRFQResMessage", ResourceUtil4Cfets.getBondRfqResMessage4Expire());
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondRFQReceiveMessage",
				ResourceUtil4Cfets.getBondRfqRecvMessage4Expired());
		/**
		 * 本方请求回复过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondRFQReplyResMessage",
				ResourceUtil4Cfets.getBondReplyResMessage4Expire());
		/**
		 * 对手请求回复过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondRFQReplyReceiveMessage",
				ResourceUtil4Cfets.getBondReplyRecvMessage4Expire());
		/**
		 * 本方做市报价过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondMarketMakingQuoteResMessage",
				ResourceUtil4Cfets.getMMMessage4Expire());
		/**
		 * 本方限价报价过期
		 */
		TYPENAME_EXPIREDMESSAGE.put("CfetsTradeBondPriceLimitQuoteResMessage",
				ResourceUtil4Cfets.getPLMessage4Expire());
		/**
		 * 本方点击成交报价过期
		 */
	 ...
	 * @return 对应报价的过期消息
	 */
	private CfetsTradeMessage getExpiredMessageTemplate(
			@SuppressWarnings("rawtypes") MockMessageInfo originMessageInfo) {

...
@Component
public class MessageInfoBuilderManager4Quote {
	
	protected Logger logger = LoggerFactory.getLogger(MessagePersistManagerDelegate.class);

	@SuppressWarnings("rawtypes")
	@Autowired
	private List<IMockMessageInfoBuilder4Quote> mockMessageInfoBuilders;
	...
	public MockMessageInfo build(Object messageTempldate, Object originMessage, OpTypeEnum optype, Date triggerTime) {
		try {
			...
			IMockMessageInfoBuilder4Quote builder = getSupportBuilder(messageTempldate.getClass(), optype);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private IMockMessageInfoBuilder4Quote getSupportBuilder(Class<?> type, OpTypeEnum optype) {
		for (IMockMessageInfoBuilder4Quote iMockMessageInfoBuilder : mockMessageInfoBuilders) {
